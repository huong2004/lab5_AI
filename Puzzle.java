	...
	public int computeH1(Node currentState) {
		...
		for (int i=0; i<MAX_ROW;i++) {
			for (int j=0;i<MAX_COL;j++) {
				int filCurent= currentState.getTile(i, j);
				int fileGoal=goalState.getTile(i, j);
				if (filCurent!=0 && filCurent!=fileGoal)
					output++;
			}
		}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			if (row >= 0) {// Tile stands inside the map
				// Tạo ra trạng thái con
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0); // Cập nhật ô trống
				result.updateTile(whiteTile[0], whiteTile[1], tmp); // Cập nhật ô số
